// Card details
let cards = [
  // Dread spot cards
  {
    id: 11,
    name: "Dread spot",
    type: "Ground",
    description: "The spooky ancient place of unholy sacrifices. Warlock masters with mana runes fought the lightforged enemy shields, smiting and purging them to the oblivium.",
    area: 4,
    capacity: 3,
    cost: 0,
    attack: NaN,
    defence: 6,
    color: "purple",
    img: "url('assets/dreadspot1.jpg')",
  },
  {
    id: 12,
    name: "Dread spot",
    type: "Ground",
    description: "The mists of forsaken souls chase away the light, making the terrain even darker and more gloomy, cornering fire mages with their treacherous tongues, corrupting even the most devout of light.",
    area: 6,
    capacity: 2,
    cost: 0,
    attack: NaN,
    defence: 4,
    color: "purple",
    img: "url('assets/dreadspot2.jpg')",
  },
  {
    id: 13,
    name: "Dread spot",
    type: "Warlock",
    description: "A vicious warrior that invokes fallen spirits and hits the enemy with their imbued runic punches. The melee attacks cause deep agony and suffering, drilling the flesh and empoisoning it.",
    area: 6,
    capacity: NaN,
    cost: 0,
    attack: 2,
    defence: 4,
    color: "purple",
    img: "url('assets/warlockdread1.jpg')",
  },
  {
    id: 14,
    name: "Dread spot",
    type: "Chilling warlock",
    description: "Chilling warlocks summon demonic energy blasts that stun the most powerful warriors. The dread greenish plasma energy is capable to smash armors and shields with only one hitting.",
    area: 6,
    capacity: NaN,
    cost: 0,
    attack: 2,
    defence: 4,
    color: "purple",
    img: "url('assets/warlockdread2.jpg')",
  },
  {
    id: 15,
    name: "Dread spot",
    type: "Stone witcher",
    description: "The shadow arrows hit the enemies and turn them to stone, making their movements difficult.",
    area: 6,
    capacity: NaN,
    cost: 0,
    attack: 2,
    defence: 4,
    color: "purple",
    img: "url('assets/witcherdread1.jpg')",
  },
  {
    id: 16,
    name: "Dread spot",
    type: "Witcher",
    description: "In its shade disguise form, the witcher ambushes behind the enemies, taking them with off guard and totally unprepadarkred.",
    area: 6,
    capacity: NaN,
    cost: 0,
    attack: 2,
    defence: 4,
    color: "purple",
    img: "url('assets/witcherdread2.jpg')",
  },
  // Holy ground cards
  {
    id: 21,
    name: "Holy ground",
    type: "Ground",
    description: "The arcane light brights over the paladins whom wield their swords. This is a place of glory and catharsis after the war and it purifies the soldier spirits.",
    area: 5,
    capacity: 3,
    cost: 0,
    attack: NaN,
    defence: 5,
    color: "darkgreen",
    img: "url('assets/holyground1.jpg')",
  },
  {
    id: 22,
    name: "Holy ground",
    type: "Ground",
    description: "The lightforged shields tinkle across the holy pathway between the ancient empowedarkred rune trees. Light warriors roar bravely with their restodarkred souls in face of battle.",
    area: 5,
    capacity: 5,
    cost: 0,
    attack: NaN,
    defence: 3,
    color: "darkgreen",
    img: "url('assets/holyground2.jpg')",
  },
  {
    id: 23,
    name: "Holy ground",
    type: "Warrior",
    description: "The warriors are trained to bravely kill their enemies with maces, swords, axes or anything else they can wield. The warfare is a constant in their lives, making their souls always alert.",
    area: 5,
    capacity: NaN,
    cost: 0,
    attack: 2,
    defence: 3,
    color: "darkgreen",
    img: "url('assets/warriorholy1.jpg')",
  },
  {
    id: 24,
    name: "Holy ground",
    type: "Holy warrior",
    description: "The holy warriors have empowedarkred armors of light that overshadow the path where they tread and confunsing the enemies sights.",
    area: 5,
    capacity: NaN,
    cost: 0,
    attack: 2,
    defence: 3,
    color: "darkgreen",
    img: "url('assets/warriorholy2.jpg')",
  },
  {
    id: 25,
    name: "Holy ground",
    type: "Holy hunter",
    description: "Holy hunters are the masters of the archery. They chase and shoot the target quickly as a lightning bolt, neutralize the threat.",
    area: 5,
    capacity: NaN,
    cost: 0,
    attack: 2,
    defence: 3,
    color: "darkgreen",
    img: "url('assets/hunterholy1.jpg')",
  },
  {
    id: 26,
    name: "Holy ground",
    type: "Wild hunter",
    description: "The wild hunters tame beasts to use them against the enemies, distracting and hitting them with fire arrows that drill everything along the way.",
    area: 5,
    capacity: NaN,
    cost: 0,
    attack: 2,
    defence: 3,
    color: "darkgreen",
    img: "url('assets/hunterholy2.jpg')",
  },
  // Bloody gorge cards
  {
    id: 31,
    name: "Bloody gorge",
    type: "Ground",
    description: "The ground was not always covered in blood, yet the gore celebrations were practiced by the insane veterans that stayed aimless wandering. Now these grounds desecrate souls and urge them on with vampiric powers.",
    area: 4,
    capacity: 2,
    cost: 0,
    attack: NaN,
    defence: 6,
    color: "darkred",
    img: "url('assets/bloodygorge1.jpg')",
  },
  {
    id: 32,
    name: "Bloody gorge",
    type: "Ground",
    description: "The bloody moon is even more red when reflecting the imbued vampiric mana ground, becoming the power-well for the forgotten underground creatures.",
    area: 5,
    capacity: 4,
    cost: 0,
    attack: NaN,
    defence: 3,
    color: "darkred",
    img: "url('assets/bloodygorge2.jpg')",
  },
  {
    id: 33,
    name: "Bloody gorge",
    type: "Blood berzerker",
    description: "A giant familiar demon guides and advises the blood berserker against the enemies. The blood berserker overwhelms everything ahead with the long sword, obliterating even the best forged armors.",
    area: 5,
    capacity: NaN,
    cost: 0,
    attack: 2,
    defence: 3,
    color: "darkred",
    img: "url('assets/berserkerbloody1.jpg')",
  },
  {
    id: 34,
    name: "Bloody gorge",
    type: "Demon tank berzerker",
    description: "Demon tank berserker is the terror of warfares. Long bloody and fiery wings exempt weapons.",
    area: 5,
    capacity: NaN,
    cost: 0,
    attack: 2,
    defence: 3,
    color: "darkred",
    img: "url('assets/berserkerbloody2.jpg')",
  },
  {
    id: 35,
    name: "Bloody gorge",
    type: "Rogue",
    description: "Rogues come out of nowhere, stabbing behind the enemy with poisoned daggers and venomous blades.",
    area: 5,
    capacity: NaN,
    cost: 0,
    attack: 2,
    defence: 3,
    color: "darkred",
    img: "url('assets/roguebloody1.jpg')",
  },
  {
    id: 36,
    name: "Bloody gorge",
    type: "Dark rogue",
    description: "Dark rogues stealthy walk within the enemy shadows, cutting throats before their warcries.",
    area: 5,
    capacity: NaN,
    cost: 0,
    attack: 2,
    defence: 3,
    color: "darkred",
    img: "url('assets/roguebloody2.jpg')",
  },
]